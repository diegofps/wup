...
#include <wup/models/addressdecoder.hpp>
...
public:
    ...
    int read_binary(const int * const pattern)
    {
        // Identifica as posicoes enderecadas
        const long long * const addresses = _addressDecoder.read(pattern);
    ...
    int read_counts(const int * const pattern)
    {
        // Identifica as posicoes enderecadas
        const long long * const addresses = _addressDecoder.read(pattern);
    ...
    int read_bleaching(const int * const pattern,
    ...
    int read_bleaching_limited(const int * const pattern,
			...
			const double minConf=0.005, const int step=1, const int limit=1000)
    {
    	minTarget = max(0, minTarget);
    	maxTarget = min(_numDiscriminators, maxTarget);

    	if (minTarget > maxTarget)
    		error("minTarget cannot be larger than maxTarget");

        const int realMax = min(limit, _maxBleaching);
        ...
        for (b=1;b<=realMax;b+=step) {
            const int predicted = _read_bleaching(pattern, b, minTarget, maxTarget);
            const double conf = confidence();

            if (conf > minConf)
                return predicted;

            if (b == 1 || conf > bestConfidence) {
                bestConfidence = conf;
    ...
    int read_bleaching_balanced(const int * const pattern, 
        const int minConf=0.1, const int step=1, const int limit=1000)
    {
        const int realMax = min(limit, _maxBleaching);
...
private:
    ...
    AddressDecoder _addressDecoder;
